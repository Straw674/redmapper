...
from redmapper.calibration import calibrate_red_sequence

class RedSequenceCalTestCase(unittest.TestCase):
    """
    Tests of redmapper.calibration.calibrate_red_sequence which calibrates the
    red sequence.
    """
    def test_redsequencecal(self):
        """
        Run tests of redmapper.calibration.calibrate_red_sequence
        """
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data_for_tests')
        ...
        calibrate_red_sequence(config, galfile, doRaise=False)
        ...
        testing.assert_allclose(pars[0]['pivotmag'], np.array([17.111357, 18.587465]), rtol=1e-3)
        ...
        testing.assert_almost_equal(pars[0]['slope01'], np.array([-0.0056, -0.0249]), 4)

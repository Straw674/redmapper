"""Functions related to preparing members for the next calibration iteration
"""
import os
import numpy as np
import esutil
...
def prep_members(config, mode, rng=None):
    """
    Prepare members for input to the next calibration iteration.

    Output members are put into config.zmemfile.
    ...
    logger.info(f"Running member preparation with mode {mode}...")
    ...
    # Cut the clusters
    use, = np.where((cat.Lambda / cat.scaleval > config.calib_minlambda) &
                    (cat.scaleval > 0.0) &
                    (np.abs(cat_z - cat.z_lambda) < config.calib_zlambda_clean_nsig * cat.z_lambda_e))
    cat = cat[use]
    cat_z = cat_z[use]

    # Cut the members
    use, = np.where((mem.p * mem.theta_i * mem.theta_r > config.calib_pcut) |
                    (mem.pcol > config.calib_pcut))
    ...
    if config.calib_smooth > 0.0:
        newmem.z[:] += config.calib_smooth * rng.normal(size=newmem.size)

    logger.info(f"Saving {newmem.size} prepared members to {config.zmemfile}")
    newmem.to_fits_file(config.zmemfile)

    # Generate QA plots if requested
    if config.more_qa_plots:
        _make_prepmembers_qa_plots(config, newmem, mode)

def _make_prepmembers_qa_plots(config, newmem, mode):
    """
    ...
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(config.plotpath, f'prepmembers_{mode}_z_scatter.png'), dpi=300)
    plt.close()

    # Plot 2: Probability distributions
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    axes[0].hist(newmem.p, bins=50, alpha=0.7, color='blue', edgecolor='black')
    axes[0].axvline(config.calib_pcut, color='red', ls='--', lw=2, label=f'pcut={config.calib_pcut}')
    ...
    axes[1].axvline(config.calib_pcut, color='red', ls='--', lw=2, label=f'pcut={config.calib_pcut}')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(config.plotpath, f'prepmembers_{mode}_probability.png'), dpi=300)
    plt.close()
    ...
    if config.nmag >= 2:
        ...
        ax.set_xlabel(f'{config.bands[0]} - {config.bands[1]} color', fontsize=12)
    else:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(config.plotpath, f'prepmembers_{mode}_cmd.png'), dpi=300)
    plt.close()
    ...
    axes[1].legend()
    plt.tight_layout()
    plt.savefig(os.path.join(config.plotpath, f'prepmembers_{mode}_central_vs_satellite.png'), dpi=300)
    plt.close()
    ...
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(config.plotpath, f'prepmembers_{mode}_z_hist.png'), dpi=300)
    plt.close()
